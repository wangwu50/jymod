...
int HAPI_LoadPNGPath(lua_State* pL)             //��ͼƬ��ȡPNG
{
    int n = lua_gettop(pL);         //�õ������ĳ���
...
int HAPI_LoadPNG(lua_State* pL)             //��ͼƬ��ȡPNG
{
    ...
    JY_LoadPNG(fileid, picid, x,y,flag,value, px, py, pw, ph, width, height, rotate,reversal,percent);
...
{
    int x = (int)lua_tonumber(pL, 1);
    char* p = (char*)lua_newuserdata(pL, x);                 //����userdata������Ҫ�ͷ��ˡ�
    int i;
...
*/
    ...
    rwops->seek(rwops,start, SEEK_SET);
    rwops->read(rwops, p, 1, length);
    rwops->close(rwops);
...
int Byte_set64(lua_State* pL)
{
    char* p = (char*)lua_touserdata(pL, 1);
    int start = (int)lua_tonumber(pL, 2);
    long long v = (long long)lua_tonumber(pL, 3);
    *(long long*)(p + start) = v;
    return 0;
}
...
int HAPI_GetPlatform(lua_State* pL)
{
    lua_pushstring(pL, "Android");
    return 1;
}

int HAPI_PlayMovie(lua_State* pL)
{
    const char* filename = lua_tostring(pL, 1);
...
int HAPI_LoadLuaFile(lua_State* pL)
{
    const char* filename = lua_tostring(pL, 1);
    int sign = (int)lua_tonumber(pL, 2);
    LoadLuaFile(filename, sign);
    return 0;
}


int HAPI_GetFileLength(lua_State* pL)
{
    const char* filename = lua_tostring(pL, 1);
...
int HAPI_RestResolution(lua_State* pL)
{
    int percent = (int)lua_tonumber(pL, 1);
    RestResolution(percent);
    return 0;
}

int HAPI_GetRealResolution(lua_State* pL)
{
    lua_pushinteger(pL, g_RealScreenW);
    lua_pushinteger(pL, g_RealScreenH);
    return 2;
}

int HAPI_GetScale(lua_State* pL)
{
    lua_pushinteger(pL, g_XScale);
    lua_pushinteger(pL, g_YScale);
    return 2;
}